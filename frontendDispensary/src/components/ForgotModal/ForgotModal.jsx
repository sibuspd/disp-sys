...
const ForgotModal = (props) => {
    ...
    const sendOTPToMail = async () => {
        ...
        // Send OTP to email
        props.showLoader();
        await axios.post('http://localhost:4000/api/auth/send-otp', {email: inputField.email})
        .then((response)=>{
    ...
    const checkOtp = async () => {
        ...
        // Verify the OTP 
        props.showLoader();
        await axios.post('http://localhost:4000/api/auth/verify-otp',{email: inputField.email, otp: inputField.otp}) //Sending email and otp from frontend input fields to backend
        .then((response)=>{
            setStep(3);
        ...
        // Update the password
        props.showLoader();
        await axios.post('http://localhost:4000/api/auth/reset-password', {email: inputField.email, newPassword: inputField.newPassword})
        .then((response) => {
            alert(response.data.message);
